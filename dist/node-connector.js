...
/***/ (function(module, exports, __webpack_require__) {
	...
	function MapdCon() {
	  ...
	  var _platform = "mapd";
	  var _nonce = 0;
	  var _balanceStrategy = "adaptive";
	  var _numConnections = 0;
	  var _lastRenderCon = 0;
	  var queryTimes = {};
	  var serverQueueTimes = null;
	  var serverPingTimes = null;
	  var pingCount = null;
	  ...
	  function processResults() {
	    ...
	    // TODO having defaulted params should be "
	   ...
	   */
	  var importTableAsync = importTableAsyncWrapper(false);
	  /**
	   ...
	   * @param {TColumnType[]} headers -- a colleciton of metadata related to the table headers
	   */
	  var importTableGeoAsync = importTableAsyncWrapper(true);
	  ...
	  function connect(callback) {
	    ...
	    var allAreArrays = Array.isArray(_host) && Array.isArray(_port) && Array.isArray(_user) && Array.isArray(_password) && Array.isArray(_dbName);
	    if (!allAreArrays) {
	    ...
	    if (!_protocol) {
	      _protocol = _host.map(function () {
	        return window.location.protocol.replace(":", "");
	      });
	    }

	    var transportUrls = getEndpoints();
	    ...
	    var _loop = function _loop(h) {
	      ...
	      client.connect(_user[h], _password[h], _dbName[h], function (error, newSessionId) {
	  ...
	  function disconnect() {
	    ...
	    if (_sessionId !== null) {
	      for (var c = 0; c < _client.length; c++) {
	        _client[c].disconnect(_sessionId[c], function (error) {
	          // eslint-disable-line no-loop-func
	  ...
	  function getFrontendViewsAsync() {
	    return new Promise(function (resolve, reject) {
	      getFrontendViews(function (error, views) {
	        if (error) {
	          reject(error);
	        } else {
	          resolve(views);
	  ...
	  function getFrontendViewAsync(viewName) {
	    return new Promise(function (resolve, reject) {
	      getFrontendView(viewName, function (err, view) {
	        if (err) {
	          reject(err);
	        } else {
	          resolve(view);
	  ...
	  function getServerStatusAsync() {
	    return new Promise(function (resolve, reject) {
	      getServerStatus(function (err, result) {
	        if (err) {
	          reject(err);
	        } else {
	          resolve(result);
	  ...
	  function createFrontendViewAsync(viewName, viewState, imageHash, metaData) {
	    ...
	    return Promise.all(_client.map(function (client, i) {
	      return new Promise(function (resolve, reject) {
	        client.create_frontend_view(_sessionId[i], viewName, viewState, imageHash, metaData, function (error, data) {
	          if (error) {
	            reject(error);
	          } else {
	            resolve(data);
	  ...
	  function deleteFrontendView(viewName, callback) {
	    ...
	    try {
	      ...
	      _client.forEach(function (client, i) {
	  ...
	  function deleteFrontendViewAsync(viewName) {
	    return new Promise(function (resolve, reject) {
	      deleteFrontendView(viewName, function (err) {
	        if (err) {
	          reject(err);
	        } else {
	          resolve(viewName);
	  ...
	  function createLinkAsync(viewState, metaData) {
	    return Promise.all(_client.map(function (client, i) {
	      return new Promise(function (resolve, reject) {
	        client.create_link(_sessionId[i], viewState, metaData, function (error, data) {
	          if (error) {
	            reject(error);
	          } else {
	  ...
	  function getLinkViewAsync(link) {
	    return new Promise(function (resolve, reject) {
	      getLinkView(link, function (err, theLink) {
	        if (err) {
	          reject(err);
	        } else {
	          resolve(theLink);
	  ...
	  function detectColumnTypesAsync(fileName, copyParams) {
	    ...
	    return new Promise(function (resolve, reject) {
	      detectColumnTypes.bind(_this3, fileName, copyParams)(function (err, res) {
	        if (err) {
	          reject(err);
	        } else {
	  ...
	  function query(sql, options, callback) {
	    ...
	    var lastQueryTime = queryId in queryTimes ? queryTimes[queryId] : DEFAULT_QUERY_TIME;
	    ...
	    try {
	      if (callback) {
	        _client[conId].sql_execute(_sessionId[conId], sql, columnarResults, curNonce, limit, function (error, result) {
	          if (error) {
	            callback(error);
	          } else {
	        ...
	        var SQLExecuteResult = _client[conId].sql_execute(_sessionId[conId], sql, columnarResults, curNonce, limit);
	  ...
	  function validateQuery(sql) {
	    return new Promise(function (resolve, reject) {
	      _client[0].sql_validate(_sessionId[0], sql, function (error, res) {
	        if (error) {
	          reject(error);
	        } else {
	  ...
	  function getTables(callback) {
	    _client[0].get_tables(_sessionId[0], function (error, tables) {
	      if (error) {
	        callback(error);
	      } else {
	  ...
	  function getTablesAsync() {
	    ...
	    return new Promise(function (resolve, reject) {
	      getTables.bind(_this4)(function (error, tables) {
	        if (error) {
	          reject(error);
	        } else {
	          resolve(tables);
	  ...
	  function getFields(tableName, callback) {
	    _client[0].get_table_details(_sessionId[0], tableName, function (error, fields) {
	      if (fields) {
	  ...
	  function createTable(tableName, rowDescObj, tableType, callback) {
	    ...
	    var thriftRowDesc = helpers.mutateThriftRowDesc(rowDescObj, importerRowDesc);

	    for (var c = 0; c < _numConnections; c++) {
	      _client[c].create_table(_sessionId[c], tableName, thriftRowDesc, tableType, function (err) {
	        if (err) {
	          callback(err);
	        } else {
	          callback();
	  ...
	  function createTableAsync(tableName, rowDescObj, tableType) {
	    return new Promise(function (resolve, reject) {
	      createTable(tableName, rowDescObj, tableType, function (err) {
	        if (err) {
	          reject(err);
	        } else {
	          resolve();
	  ...
	  function importTable(tableName, fileName, copyParams, rowDescObj, isShapeFile, callback) {
	    ...
	    for (var c = 0; c < _numConnections; c++) {
	      if (isShapeFile) {
	        _client[c].import_geo_table(_sessionId[c], tableName, fileName, thriftCopyParams, thriftRowDesc, thriftCallBack);
	      } else {
	        _client[c].import_table(_sessionId[c], tableName, fileName, thriftCopyParams, thriftCallBack);
	      }
	    }
	  }
	  ...
	  function importTableAsyncWrapper(isShapeFile) {
	    return function (tableName, fileName, copyParams, headers) {
	      return new Promise(function (resolve, reject) {
	        importTable(tableName, fileName, copyParams, headers, isShapeFile, function (err, link) {
	          if (err) {
	            reject(err);
	          } else {
	            resolve(link);
	  ...
	  function renderVega(widgetid, vega, options, callback) /* istanbul ignore next */{
	    ...
	    try {
	      if (!callback) {
	        var renderResult = _client[conId].render_vega(_sessionId[conId], widgetid, vega, compressionLevel, curNonce);
	        return processResults(processResultsOptions, renderResult);
	      }

	      _client[conId].render_vega(_sessionId[conId], widgetid, vega, compressionLevel, curNonce, function (error, result) {
	        if (error) {
	          callback(error);
	        } else {
	          processResults(processResultsOptions, result, callback);
	  ...
	  function getResultRowForPixel(widgetId, pixel, tableColNamesMap, callbacks) /* istanbul ignore next */{
	    ...
	    try {
	      if (!callbacks) {
	        return processPixelResults(undefined, // eslint-disable-line no-undefined
	        _client[_lastRenderCon].get_result_row_for_pixel(_sessionId[_lastRenderCon], widgetId, pixel, tableColNamesMap, columnFormat, pixelRadius, curNonce));
	      }
	      _client[_lastRenderCon].get_result_row_for_pixel(_sessionId[_lastRenderCon], widgetId, pixel, tableColNamesMap, columnFormat, pixelRadius, curNonce, processPixelResults.bind(this, callbacks));
	    } catch (err) {
	      throw err;
	    }
	    return curNonce;
	  ...
	  function getEndpoints() {
	    return _host.map(function (oldHost, i) {
	      return _protocol[i] + "://" + oldHost + ":" + _port[i];
	    });
	  }
	}
